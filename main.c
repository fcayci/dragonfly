 ...
 * Author  : Furkan Cayci
 *
 * DragonFly Quadcopter Hardware RevA (May 2013)
 * Configuration:
 *
...
/* USART definitions
 * 115200 - 8-bits - Even parity - 1 stop
 */
...
static double errorX, integralX, derivativeX;
static int16_t outputX, outputY;
static double errorY, integralY, derivativeY;
...
static void motorsTest(void){
	uint8_t i;
	for (i = 0; i<4; i++)
		motorsSetSpeed(i, MAX_MOTOR_SPEED/50);
		chThdSleepMilliseconds(5000);
		motorsSetSpeed(i, 0);
}
